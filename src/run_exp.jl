using Distributed, DataStructures, Serialization, ArgParse, Dates
...
@everywhere function execute_task(task)
	...
	iterations = simulation_months * DAYS_IN_MONTH

	model = initialize(task[:properties])
	agent_data, model_data = run!(model, iterations; adata = adata, mdata = mdata, when=when_collect)
...
mkpath(folder_data)
serialize("$folder_data/data-$chunk.dat", results)
